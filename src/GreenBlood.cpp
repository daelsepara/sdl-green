    ...
    if (window && renderer)
    {
        ...
        if (font)
        {
            while (!done)
            {
                ...
                renderButtons(renderer, controls, current, intGN, space, space / 2);
...
bool glossaryScreen(SDL_Window *window, SDL_Renderer *renderer, std::vector<Skill::Base> Skills)
{
    ...
    if (window && renderer)
    {
        ...
        while (!quit)
        {
            ...
            renderButtons(renderer, controls, current, intGN, border_space, border_pts, (offset > 0), glossary && offset < (glossary->h - text_bounds + 2 * space));
...
bool inventoryScreen(SDL_Window *window, SDL_Renderer *renderer, Character::Base &player, Story::Base *story, std::vector<Item::Base> &Items, Control::Type mode, int limit)
{
    if (Items.size() > 0)
    {
        ...
        while (!done)
        {
            ...
            renderButtons(renderer, controls, current, intGN, text_space, text_space / 2);
...
void renderAdventurer(SDL_Window *window, SDL_Renderer *renderer, TTF_Font *font, Character::Base &player)
{
    ...
    putText(renderer, name_string.c_str(), font, space, clrWH, intGN, TTF_STYLE_NORMAL, headerw, headerh, startx, starty);
...
Character::Base customCharacter(SDL_Window *window, SDL_Renderer *renderer)
{
    ...
    if (window && renderer)
    {
        ...
        while (!done)
        {
            ...
            renderButtons(renderer, controls, current, intGN, 8, 4);
...
bool aboutScreen(SDL_Window *window, SDL_Renderer *renderer)
{
    ...
    // Render the image
    if (window && renderer && splash && background && text)
    {
        ...
        while (!done)
        {
            ...
            fillRect(renderer, text->w + 2 * text_space, text->h + 2 * text_space, startx * 2 + splashw, starty, intGN);

            renderText(renderer, text, intGN, startx * 2 + splashw + text_space, starty + text_space, SCREEN_HEIGHT * (1.0 - 2 * Margin) - 2 * text_space, 0);
...
std::vector<Button> createFilesList(SDL_Window *window, SDL_Renderer *renderer, std::vector<std::string> list, int start, int last, int limit, bool save_button)
{
    ...
    if (list.size() > 0)
    {
        for (int i = 0; i < last - start; i++)
        {
            ...
            auto button = createHeaderButton(window, game_string.c_str(), clrWH, intGN, textwidth - 3 * button_space / 2, 0.125 * SCREEN_HEIGHT, text_space);
...
Control::Type gameScreen(SDL_Window *window, SDL_Renderer *renderer, Character::Base &player, bool save_botton)
{
    ...
    if (window && renderer)
    {
        ...
        while (!done)
        {
            ...
            renderButtons(renderer, controls, current, intGN, border_space, border_pts);
    ...
    while (!quit)
    {
        ...
        if (window && renderer)
        {
            ...
            while (!quit)
            {
                ...
                renderButtons(renderer, controls, current, intGN, border_space, border_pts, (offset > 0), text && offset < (text->h - text_bounds + 2 * space));
...
bool mainScreen(SDL_Window *window, SDL_Renderer *renderer, int storyID)
{
    ...
    if (window && renderer && splash && text)
    {
        ...
        while (!done)
        {
            ...
            fillRect(renderer, text->w + 2 * text_space, text->h + 2 * text_space, startx * 2 + splashw, texty, intGN);
